...
size_t size1 = 1; // Numero de imagens de amostra. Tamanho do vector
std::vector<Mat_<FLT>>
    imagens_referencia(size1); // Cria vetor para imagens de amostra
...
int melhor_template_nnorm(
    int ultimo_tamanho) { // Acha o tamanho ideal da imagem de referencia
  ...
  for (int ref = 0; ref < size1;
       ref++) { // Percorre todas as imagens de amostra
    ...
    for (int i = minimo; i < maximo;
      ...
      // cout << " i = " << i;
      resize(imagens_referencia[ref], imagem_referencia_temp, Size(i, i), 0, 0,
             INTER_AREA);

      imagem_referencia_temp = somaAbsDois(dcReject(imagem_referencia_temp));

      matchTemplate(imagem_original_flt, imagem_referencia_temp, resultado,
                    CV_TM_CCORR);
...
void desenha_retangulo(int tamanho) {
  ...
  line(imagem_original,
       Point(matchLoc.x + templ.cols / 2,
             matchLoc.y + templ.rows / 2 + 0.02 * templ.rows),
       Point(matchLoc.x + templ.cols / 2,
             matchLoc.y + templ.rows / 2 - 0.02 * templ.rows),
       Scalar(0, 255, 255), 1, 8);

  line(imagem_original,
       Point(matchLoc.x + templ.cols / 2 + 0.02 * templ.rows,
             matchLoc.y + templ.rows / 2),
       Point(matchLoc.x + templ.cols / 2 - 0.02 * templ.rows,
             matchLoc.y + templ.rows / 2),
       Scalar(0, 255, 255), 1, 8);

  rectangle(imagem_original, matchLoc,
...
int main(int argc, char *argv[]) {
  ...
  if (argc != 4) {
    ...
    int novo_tamanho =
        0; // Variavel que definira melhor tamanho para o tamplate matching
    ...
    for (int i = 1; i <= frames; i++) {
      ...
      // Acha o template melhor se encaixa na imagem e posteriormente o tamanho
      // de tal template
      ...
      Mat_<FLT> imagem_referencia_temp;
      resize(imagens_referencia[0], imagem_referencia_temp,
             Size(novo_tamanho, novo_tamanho), 0, 0, INTER_AREA);
      imagem_referencia_temp = somaAbsDois(dcReject(imagem_referencia_temp));

      matchTemplate(imagem_original_flt, imagem_referencia_temp, resultado,
                    CV_TM_CCORR);

      // Encontra o ponto em que a semelhanca foi maxima e com isso determina o
      // centro do circulo a ser desenhado na placa
      minMaxLoc(resultado, &minVal, &maxVal, &minLoc, &maxLoc);

      // se a primeira correlação for maior que 0.18
      if (imagem_ultimo_frame == true) {
        if (((maxLoc.x - last_loc.x) * (maxLoc.x - last_loc.x) +
             (maxLoc.y - last_loc.y) * (maxLoc.y - last_loc.y)) < 400 &&
            maxVal > 0.10) {

          resize(imagens_referencia[0], imagem_referencia_temp,
                 Size(novo_tamanho, novo_tamanho), 0, 0, INTER_AREA);
          matchTemplate(imagem_original_flt, imagem_referencia_temp,
                        resultado_norm, CV_TM_CCOEFF_NORMED);
          ...
          if (((maxLoc.x - maxLoc_norm.x) * (maxLoc.x - maxLoc_norm.x) +
            ...
            last_loc = matchLoc;
            desenha_retangulo(novo_tamanho); // Funcao que desenha retangulo na
                                             // imagemd e saida
            // cout << maxLoc << " - ";
          } else {
