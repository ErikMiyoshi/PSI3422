...
void extrai_numero(int tamanho, int xi, int yi, int xf, int yf){
    ...
    if ((xf-xi) > 14 && ((yf-yi) > 14)){
        ...
        for (int l = xi; l < xf; l++){
            for (int c = yi; c < yf; c++)
            {
                numero_extraido(c - yi, l - xi) = imagem_original(c, l);   
        ...
        for (int l = 0; l < imagem_numero_flt.rows; l++){
            for (int c = 0; c < imagem_numero_flt.cols; c++)
            {
                ...
                if (imagem_numero_flt(l,c) < 0.35){
                    imagem_numero_flt(l,c) = 0.0;
                    calcula_bbox = true;
                    calcula_numero = true;
                } else if (imagem_numero_flt(l,c) > 0.5){
...
void desenha_retangulo(int tamanho){
		...
		rectangle( imagem_original,  Point( matchLoc.x + 10 , matchLoc.y + 10 ), 
...
int main(int argc, char* argv[]) {  
    ...
    // Programa Principal
        ...
        mnist.le("C:\\Users\\marco\\Desktop\\Poli\\Materias\\Lab de Sistemas eletronicos\\Parte2\\Fase5\\mnist");
        ...
        for (int i = 1; i <= frames; i++){
            ...
            Mat_<COR> imagem_original_aux(imagem_original.rows, imagem_original.cols, COR(0,0,255)); ;
                ...
                if (((maxLoc.x - last_loc.x) * (maxLoc.x - last_loc.x) + 
                    ...
                    if (((maxLoc.x - maxLoc_norm.x) * (maxLoc.x - maxLoc_norm.x) + 
                        ...
                        if(calcula_numero){
                            ...
                            putTxt(imagem_original, 14, 28, to_string(qp2));
                        }
